;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;                                                             ;
; NetLogo Code: Program to model bacterial population dynamics;
;                                                             ;
;                                                             ;
;                                                             ;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
; NOTES: Globals are set here. "globals" is like "set" keyword but for global variables
; NOTES: Only the setup, go one step, and go buttons really execute code.
;        The other functions written are called into the main functions setup, go one step, and go

globals [ total_nutrient total_end_product total_biomass upt ava_upt       ; "globals" - keyword for setting "global" variables
  bacteria_t-1 bacteria_t biomass_t-1 biomass_t rate_bacteria rate_biomass
  devr devu devv min_mass devmm]
turtles-own [ mass start_mass energy viab ]      ; "turtles-own" - defines the variables belonging to each patch.
patches-own [ local_nutrient local_end_product ] ; "patches-own" - defines the variables belonging to each patch.


;; INITIALIZATION

; NOTES: Syntax is similar to lisp, but is written in Scala and Java (has some "lisp" like qualities,
;        that can be seen in the "operator first" syntax).
;
; Functions: The language has a weird relationship, with functions, in comparison with C.
;            It has no main function. Functions can be "assigned" to a button in the "Interface" portion of the NetLogo software.
;            Still, some aspects are familiar. Once a function is defined it can be used in another function (see "go" function).

; Key concepts: set - used to "initialize" some variable to some value
;               ask - prompts "turtle or patches" to execute some direction
;
; Interface: The interface is where buttons/sliders and graphs/plots are created and populated. Values can be populated
;            by the aforementioned buttons/sliders, and then used in the actual simulation. This allows for ease and "user friendly"
;            use by the computationally challenged (i.e. non-computer scientist)

;; Function declaration "setup"
to setup
  clear-all                              ; code for clearing any previous "runs" of the code present in the NetLogo windo
  random-seed 1111
  set devr 0.20 * mass_reproduction      ; Slider Button Value: "mass_reproduction"
  set devu 0.15 * uptake_k               ; Slider Button Value: "uptake_k"
  set devv 0.20 * viability_time         ; Slider Button Value: "viability_time
  set min_mass 0.50 * mass_reproduction
  set devmm 0.25 * min_mass              ; Slider Button Value: "min_mass"

  setup-bacteria                         ; function call to "setup-bacteria"

  set total_nutrient 0                   ; setting/resetting global variable "total_nutrient" to zero
  set total_end_product 0                ; setting/resetting global variable "total_end_product" to zero

  setup-medium                           ; function call to "setup-medium"

  do-plotting                            ; function call to "do-plotting"
  reset-ticks                            ; reset the "counter" for initial setup
end

;; Function declaration "setup-bacteria"
to setup-bacteria
  set-default-shape turtles "dot"   ; setting default shape of "turtle"
    create-turtles initial_bacteria ; using value from "slider button" (i.e. initial_bacteria) to generate number of turtles
  [ setxy random-pxcor random-pycor ; setting up "x and y" coordinates of patches with a "random" function
...
;; Function declaration "setup-medium"
to setup-medium
 ask patches [ set local_nutrient initial_local_nutrient] ; ask command - prompts "turtles" or "patches" to execute directions in [input] brackets
...
; NOTE: Used in "go" button and "go one step"
...
to go
  ...
  ask turtles [ move uptake viability reproduce ] ; uses "move", "uptake", "viability", and "reproduce" functions

  set total_biomass sum [mass] of turtles
  set total_nutrient sum [local_nutrient] of patches
  set total_end_product sum [local_end_product] of patches

  update_system                                   ; function call to "update_system"
  ...
  do-plotting
    ...
    ;open-file         ; NOTE: uncomment this command to use "INDISIM1.nlogo" without the program
    ;  write-to-file   ; NOTE: uncomment this command to restore normal "write" functionality at every tick of simulation
  ;close-file        ; NOTE: uncomment this command to use "INDISIM1.nlogo" without the program

end


to move                        ; Function Definition: "move"
  move-to one-of neighbors
end

to uptake                      ; Function Definition: "uptake"
...
to update_system                ; Funciton Definition: "update_system"
  diffuse local_nutrient 0.50
  diffuse local_end_product 0.50

;;Protocol of the bioreactor

 if (bioreactor = "Fed-batch")  ; control flow structure for "Bioreactor" button
  [
...
to do-plotting                            ; Function Definition: "do-plotting"
  ...
  plot count turtles                      ; plotting turtles on "Viable bacteria" plot
  ...
  histogram [mass] of turtles             ; histogram actually depicts turtles
end
...
to open-file
  ...
  let file "INDISIM1_output.txt"   ; declaring file named "INDISIM1.txt"
  file-open file           ; open file

end

to close-file              ; NOTE: must close file at END of run or NO DATA will be written!!!!!!

  file-open "INDISIM1_output.txt"
  file-close

end

to write-to-file
  if ticks <= 2000 [file-print ( word " " ticks  word "  " count turtles)]
  ;file-print ""
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"mass_distribution" 2.0 1 -16777216 true "" ""
