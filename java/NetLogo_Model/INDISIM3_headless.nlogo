...
to setup
  clear-all
  random-seed 1111
  set devr 0.20 * mass_reproduction
  set devu 0.15 * uptake_k
  set devv 0.20 * viability_time
  set min_mass 0.50 * mass_reproduction
  set devmm 0.25 * min_mass
...
to setup-bacteria-1
  set-default-shape turtles "dot"
    create-turtles initial_bacteria_1 ;; PRODUCER BACTERIA
  [ setxy random-pxcor random-pycor
    ...
    set energy 0
    set viab 0
    set color pink                    ;; PRODUCERS = PINK BACTERIA
    set total_biomass total_biomass + mass]

  create-turtles initial_bacteria_2  ;; CONSUMER BACTERIA
  [ setxy random-pxcor random-pycor
    let mass_ini mass_reproduction / 2
    set mass abs ( random-normal mass_ini devr )
    set start_mass abs ( random-normal mass_reproduction devr )
    set energy 0
    set viab 0
    set color sky                    ;; CONSUMERS = SKY BLUE BACTERIA
    set total_biomass total_biomass + mass]
end
...
to go
  ...
  ask turtles [ move uptake eat-ex viability reproduce ] ;; TURTLES EXECUTE ACTIONS (e.g. eat,move,reproduce, die)
...
to eat-ex
  ...
  set ava_upt availability_k * [local_end_product] of patch-here
  if upt > ava_upt [set upt ava_upt]

  set local_end_product local_end_product - upt

  set energy yield-2 * upt
  set energy energy - maintenance_k * mass - inhibitory_k * local_nutrient

  if energy < 0 [set energy 0 if mass > abs ( random-normal min_mass devmm ) [set mass (0.99 * mass)]]
  set mass mass + yield-2 * energy ;; production of new biomass
  ]
end
...
to-report data
  ...
  report initial_bacteria_1
  report initial_bacteria_2
  report count turtles with [color = pink]
  report count turtles with [color = sky]
end
...
"" ""
PENS
"Consumers" 1.0 0 -13791810 true "" "plot count turtles with [ color = sky ]"
"Producers" 1.0 0 -2064490 true "" "plot count turtles with [ color = pink ]"
